...
function BarChart({ data, labels, color }) {
  ...
  return React.createElement('div', { style: { display: 'flex', alignItems: 'flex-end', gap: 8, height: 180, padding: '0 4px' } },
    data.map((v, i) => React.createElement('div', { key: i, style: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 } },
      React.createElement('div', { style: { width: '100%', maxWidth: 40, height: (v / max * 140) + '%', background: color || 'linear-gradient(180deg, #3b82f6, #8b5cf6)', borderRadius: '6px 6px 0 0', transition: 'height 0.5s ease', minHeight: 4, position: 'relative' } },
...
function App() {
  ...
  const [team] = useState([
    { name: 'Sarah Chen', role: 'Product Designer', dept: 'Design', avatar: 'SC', color: '#3b82f6', tasks: 3, online: true },
    { name: 'Mike Johnson', role: 'Senior Developer', dept: 'Engineering', avatar: 'MJ', color: '#8b5cf6', tasks: 5, online: true },
    { name: 'Emma Wilson', role: 'Frontend Dev', dept: 'Engineering', avatar: 'EW', color: '#06b6d4', tasks: 4, online: false },
  ...
  const [activity] = useState([
    { user: 'Sarah Chen', action: 'completed task "Redesign landing page"', time: '5 min ago', avatar: 'SC', color: '#3b82f6' },
    { user: 'Mike Johnson', action: 'commented on "Fix payment bug #234"', time: '12 min ago', avatar: 'MJ', color: '#8b5cf6' },
    { user: 'Emma Wilson', action: 'created task "Mobile app testing"', time: '1 hour ago', avatar: 'EW', color: '#06b6d4' },
  ...
  const kpis = [
    { label: 'Total Revenue', value: '$' + totalRevenue + 'k', change: '+12.5%', up: true, icon: '💰', color: '#3b82f6' },
    { label: 'Active Users', value: '24,580', change: '+8.2%', up: true, icon: '👥', color: '#8b5cf6' },
    { label: 'Conversion Rate', value: '4.7%', change: '+0.3%', up: true, icon: '🎯', color: '#06b6d4' },
  ...
  return React.createElement('div', { style: { minHeight: '100vh', background: 'linear-gradient(135deg, #f8fafc 0%, #eef2ff 50%, #f5f3ff 100%)', fontFamily: 'Inter, system-ui, sans-serif', display: 'flex' } },
    ...
    React.createElement('div', { style: { width: 240, height: '100vh', position: 'sticky', top: 0, background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(20px)', borderRight: '1px solid rgba(241,245,249,0.8)', display: 'flex', flexDirection: 'column', flexShrink: 0 } },
      React.createElement('div', { style: { padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 10 } },
        React.createElement('div', { style: { width: 32, height: 32, borderRadius: 10, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center' } }, React.createElement('span', { style: { color: 'white', fontWeight: 800, fontSize: 16 } }, 'N')),
      ...
      React.createElement('div', { style: { padding: 16 } },
        React.createElement('div', { style: { background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: 16, padding: 16, color: 'white' } },
          ...
          React.createElement('button', { style: { width: '100%', padding: '8px', borderRadius: 10, border: 'none', background: 'white', color: '#3b82f6', fontSize: 12, fontWeight: 700, cursor: 'pointer' } }, 'Upgrade')
        )
      )
    ),
    ...
    React.createElement('div', { style: { flex: 1, padding: '24px 32px', overflowY: 'auto', maxHeight: '100vh' } },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: 12, alignItems: 'center' } },
          ...
          React.createElement('button', { style: { padding: '10px 20px', borderRadius: 999, border: 'none', background: '#3b82f6', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer' } }, '+ New Report')
        )
      ),
        ...
        React.createElement('div', { style: { ...cardStyle, overflow: 'hidden' } },
          ...
          React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
            ...
            React.createElement('tbody', null, transactions.map(t => React.createElement('tr', { key: t.id, style: { borderBottom: '1px solid #f8fafc' } },
              React.createElement('td', { style: { padding: '14px 24px', fontSize: 13, fontWeight: 600, color: '#3b82f6' } }, t.id),
      ...
      view === 'analytics' && React.createElement('div', null,
        ...
        React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 } },
          React.createElement('div', { style: { ...cardStyle, padding: 24 } },
            ...
            [{ s: 'Organic Search', v: 45, c: '#3b82f6' }, { s: 'Direct', v: 25, c: '#8b5cf6' }, { s: 'Social Media', v: 18, c: '#06b6d4' }, { s: 'Referral', v: 12, c: '#f59e0b' }].map(t => React.createElement('div', { key: t.s, style: { marginBottom: 14 } },
          ...
          React.createElement('div', { style: { ...cardStyle, padding: 24 } },
            ...
            [{ p: '/dashboard', v: '342k views' }, { p: '/pricing', v: '128k views' }, { p: '/features', v: '98k views' }, { p: '/blog', v: '76k views' }, { p: '/about', v: '45k views' }].map((p, i) => React.createElement('div', { key: i, style: { display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f1f5f9' } },
              React.createElement('span', { style: { fontSize: 13, fontWeight: 600, color: '#3b82f6' } }, p.p),
      ...
      view === 'tasks' && React.createElement('div', null,
        React.createElement('div', { style: { display: 'flex', gap: 8, marginBottom: 20 } },
          ...
          React.createElement('button', { onClick: addTask, style: { padding: '12px 24px', borderRadius: 14, border: 'none', background: '#3b82f6', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer' } }, '+ Add')
        ),
              ...
              colTasks.map(t => React.createElement('div', { key: t.id, style: { background: 'white', borderRadius: 14, padding: 16, marginBottom: 10, border: '1px solid #f1f5f9', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' } },
                ...
                React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
                  ...
                  React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
                    React.createElement('div', { style: { width: 24, height: 24, borderRadius: 8, background: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 10, fontWeight: 700 } }, t.assignee.charAt(0))
                  )
                ),
                React.createElement('div', { style: { display: 'flex', gap: 6, marginTop: 10 } },
                  ...
                  col.id !== 'done' && React.createElement('button', { onClick: () => moveTask(t.id, 1), style: { flex: 1, padding: '5px', borderRadius: 8, border: 'none', background: '#3b82f6', cursor: 'pointer', fontSize: 11, color: 'white', fontWeight: 600 } }, 'Forward →')
                )
              ))
            );
          })
      ...
      view === 'team' && React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 } },
        team.map(m => React.createElement('div', { key: m.name, style: { ...cardStyle, padding: 24 } },
          ...
          React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '1px solid #f1f5f9' } },
            ...
            React.createElement('div', { style: { textAlign: 'right' } }, React.createElement('p', { style: { fontSize: 11, color: '#94a3b8', margin: 0 } }, 'Active Tasks'), React.createElement('p', { style: { fontSize: 13, fontWeight: 600, color: '#3b82f6', margin: '2px 0 0' } }, m.tasks))
          )
        ))
      ),
      ...
      view === 'settings' && React.createElement('div', { style: { maxWidth: 600 } },
        React.createElement('div', { style: { ...cardStyle, padding: 32, marginBottom: 16 } },
          ...
          React.createElement('div', { style: { display: 'flex', gap: 24, marginBottom: 24 } },
            React.createElement('div', { style: { width: 80, height: 80, borderRadius: 20, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 28 } }, 'DR'),
          ...
          React.createElement('button', { style: { padding: '12px 32px', borderRadius: 12, border: 'none', background: '#3b82f6', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer' } }, 'Save Changes')
        ),
        React.createElement('div', { style: { ...cardStyle, padding: 32 } },
          ...
          [{ l: 'Email Notifications', d: 'Receive email about activity' }, { l: 'Push Notifications', d: 'Receive push notifications' }, { l: 'Weekly Reports', d: 'Get weekly summary reports' }, { l: 'Dark Mode', d: 'Enable dark theme' }].map((s, i) => React.createElement('div', { key: i, style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid #f1f5f9' } },
            ...
            React.createElement('div', { onClick: e => { e.currentTarget.style.background = e.currentTarget.style.background === 'rgb(59, 130, 246)' ? '#e2e8f0' : '#3b82f6'; }, style: { width: 44, height: 24, borderRadius: 999, background: i < 2 ? '#3b82f6' : '#e2e8f0', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' } },
