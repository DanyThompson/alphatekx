...
function updateMission(id: string, patch: Partial<Omit<Mission, 'id' | 'createdAt'>>) {
...
export function saveCreation(input: { missionId: string; title: string; code: string; type?: string; files?: CreationFile[] }): Creation {
...
async function cloudUpsertCreation(creation: Creation) {
  const userId = await currentUserId()
  if (!supabase || !userId) return
  await supabase.from('creations').upsert({ id: creation.id, mission_id: creation.missionId, user_id: userId, title: creation.title, code: creation.code, type: creation.type, status: creation.status, files: creation.files, versions:creation.versions??[], custom_domain:creation.customDomain??null, published: creation.published, deployment_url: creation.deploymentUrl ?? null, created_at: creation.createdAt })
}
